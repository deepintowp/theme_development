...
function theme_one_contact_cpt() {
	...
	add_action( 'add_meta_boxes', 'themeone_contact_add_meta_box' );
	add_action('save_post', 'save_contact_all_team_meta');
}
add_action( 'init', 'theme_one_contact_cpt'  );
...
// Add meta box in tem cpt
function themeone_contact_add_meta_box() {
	add_meta_box( 'user', 'Contact Information', 'themeone_conatct_form_meta_callback', 'contact');
}

function themeone_conatct_form_meta_callback(){
	wp_nonce_field(basename(__FILE__), 'themeone_contact_none_meta');
	$all_value = get_post_meta(get_the_ID());
	?>
	...
	<div class="metboxes_container">
			...
			<div class="single_metbox" style="margin:8px 0; padding:10px 0;     font-size: 18px; font-weight: 700; vertical-align: top; margin: 5px 7px 0 0;" >
				...
				<input style="width: 80%;  float:right;  border: 1px solid #999;"type="email" id="contact_email" name="contact_email" value="<?php 
					if($all_value['contact_email']){
					echo $all_value['contact_email'][0];
					}
				?>" />
			</div><!-- single_metbox -->
			<div class="single_metbox" style="margin:8px 0; padding:10px 0;     font-size: 18px; font-weight: 700; vertical-align: top; margin: 5px 7px 0 0;" >
				<label for="contact_phone">Phone</label>
				<input style="width: 80%;  float:right;  border: 1px solid #999;"type="text" id="contact_phone" name="contact_phone" value="<?php 
					if($all_value['contact_phone']){
					echo $all_value['contact_phone'][0];
					}
				?>" />
			</div><!-- single_metbox -->
			
			<div class="single_metbox" style="margin:8px 0; padding:10px 0;     font-size: 18px; font-weight: 700; vertical-align: top; margin: 5px 7px 0 0;" >
				<label style="vertical-align: top;" for="contact_msg">Description</label>
				<textarea style="width: 80%; float:right;" name="contact_msg" id="contact_msg" cols="30" rows="10"><?php 
					if($all_value['contact_msg']){
					echo $all_value['contact_msg'][0];
					}
				?></textarea>
...
function save_contact_all_team_meta(){
		$autosave = wp_is_post_autosave(get_the_ID());
		$revision = wp_is_post_revision(get_the_ID());
		if($autosave || $revision ){
		return;
		}
		if(!wp_verify_nonce($_POST['themeone_contact_none_meta'], basename(__FILE__))){
		return;
		}
		if(isset($_POST['contact_email'])){
		update_post_meta(get_the_ID(), 'contact_email', sanitize_text_field($_POST['contact_email']));
		}
		
		if(isset($_POST['contact_phone'])){
		update_post_meta(get_the_ID(), 'contact_phone', sanitize_text_field($_POST['contact_phone']));
		}
		if(isset($_POST['contact_msg'])){
		update_post_meta(get_the_ID(), 'contact_msg', sanitize_text_field($_POST['contact_msg']));
		}
